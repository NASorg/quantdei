...
getwd()
setwd("C:/Users/THIS IS AN EXAMPLE/Desktop/dei_in_journals/data_clean")
...
# Load MedLine Data
medline <- read.csv("medline_clean.csv")

# Search For Duplicate Entries
medline_0 <- medline %>% distinct(title, .keep_all = T)

# Filter MedLine Results By Title:
medline_titles <- medline_0 %>% 
  filter((
...
# Search MedLine Abstracts / Step One:
medline_title_fail <- medline_0 %>% filter((
  str_detect(title,"diversity")| str_detect(title,"Diversity")|
    str_detect(title,"equity")| str_detect(title,"Equity")|
    str_detect(title,"health equity")| str_detect(title,"Health Equity")|
    str_detect(title,"inclusion")| str_detect(title,"Inclusion")|
...
# Search MedLine Abstracts / Step Two:
medline_abstracts <- medline_title_fail %>% filter((
...
# Bind Medline Positive Titles & Positive Abstracts
medline_actual <- rbind(medline_titles, medline_abstracts)

# See Sources of Articles
medline_journal <- medline_actual %>% get_dupes(journal)
medline_other_journals <- anti_join(medline_actual, medline_journal)
...
medline_actual %>%
  ...
  scale_x_continuous(breaks = pretty_breaks(), limits = c(2008, 2022)) +
  labs(title = "DEI Related Articles Published in MedLine Database By Year (2008-2022)",
       x = "Time", y = "Count") +
...
medline_year <- medline_actual %>% get_dupes(year)